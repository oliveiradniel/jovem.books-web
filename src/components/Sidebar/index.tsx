import { useSidebar } from './context/use';
import { useState, type ComponentProps, type CSSProperties } from 'react';

import { Slot } from 'radix-ui';

import { sidebarMenuButtonVariants } from './context/variants';
...
function Sidebar({
      ...
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        >
          <SheetHeader className="sr-only">
            ...
            <SheetDescription>
              Exibe a barra lateral para dispositivos móveis.
            </SheetDescription>
          </SheetHeader>
  ...
  return (
    <div
      ...
      data-slot="sidebar"
    >
      {/* É isso que resolve o problema do espaço na barra lateral na versão desktop. */}
      <div
      ...
      <div
        ...
        data-side={side}
        className={cn(
          'inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-200 ease-linear data-[side=left]:left-0 data-[side=left]:group-data-[collapsible=offcanvas]:-left-(--sidebar-width) data-[side=right]:right-0 data-[side=right]:group-data-[collapsible=offcanvas]:-right-(--sidebar-width) md:flex',
          // Ajustar o espaçamento para as variantes flutuantes e embutidas.
          variant === 'floating' || variant === 'inset'
...
function SidebarMenu({ className, ...props }: ComponentProps<'ul'>) {
  return (
    <ul
      ...
      data-sidebar="menu"
      className={cn(
        'flex w-full min-w-0 flex-col gap-0 group-data-[state=collapsed]:items-center',
        className,
      )}
      {...props}
    />
...
function SidebarMenuSubButton({
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        'text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent/10 flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 outline-hidden group-data-[collapsible=icon]:hidden focus-visible:ring-1 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[size=md]:text-sm data-[size=sm]:text-xs [&>span:last-child]:truncate [&>svg]:size-5 [&>svg]:shrink-0',
        className,
      )}
      {...props}
    />
